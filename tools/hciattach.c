...
struct uart_t uart[] = {
    ...
    /*Qualcomm QSoC 8x50, a copy of any */
...
int main(int argc, char *argv[])
{
	...
	alarm(to);
	bcsp_max_retries = to;
	/* download firmware, must be done before init_uart() */
        if (!strcmp(u->type, "qualcomm")) {
		n = system("hci_qcomm_init");
		if (n != 0) {
			fprintf(stderr, "QSoC firmware download failed");
			exit(1);
		}
	}
